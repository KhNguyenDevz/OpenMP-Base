...
stock LoadModelSelectionMenu(f_name[])
{
	...
	f = fopen(str, io_read);
	if( !f ) {
		Log("mSelection", "-mSelection- WARNING: Khong the tai danh sach: \"%s\"");
		return mS_INVALID_LISTID;
	}

	if(gListAmount >= mS_TOTAL_LISTS)
	{
		Log("mSelection","-mSelection- WARNING: Reached maximum amount of lists, increase \"mS_TOTAL_LISTS\"");
		return mS_INVALID_LISTID;
	}
	...
	Log("mSelection.log", "-mSelection- WARNING: Khong co item duoc tim thay trong tap tin: %s");
	return mS_INVALID_LISTID;
}
